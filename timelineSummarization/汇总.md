...
时间检测上：有均匀分时间，更恰当的为不均匀分时间。

不均匀时间切分：

1-Timeline Summarization from Social Media with Life Cycle Models----情节检测 life cycle model

7-Generating timeline summaries with social media attention----用户评论活动的爆点作为一个时间点。

摘要生成：丰富性information + popularity + 

想法：改进点--时间点分布+时间点之间相关性+生成式方法

时间点分布：life cycle model + 用户联合（内容+点赞+转发）



1 Timeline Summarization from Social Media with Life Cycle Models

情节检测+时间轴摘要：life cycle model+learn to rank

问题：

1 life cycle model太复杂，是否有精简的方法。

2 本文时间轴检测和摘要生成式两个独立的阶段，可否联合。
